...
public:
    OrbitalElements parse(const std::string& filepath) override {
        ...
        while (std::getline(file, line)) {
            // Robust line parsing (handling leading spaces)
            std::string trimmed = line;
            trimmed.erase(0, trimmed.find_first_not_of(" \t"));
            
            if (trimmed.compare(0, 3, "EQU") == 0) {
                std::istringstream iss(trimmed.substr(3));
                iss >> eq_elem.a >> eq_elem.h >> eq_elem.k 
                    >> eq_elem.p >> eq_elem.q >> eq_elem.lambda;
                found_equ = true;
                std::cout << "[EQ1Parser] Found EQU: a=" << eq_elem.a << " h=" << eq_elem.h << " k=" << eq_elem.k << std::endl;
            }
            else if (trimmed.compare(0, 3, "MJD") == 0) {
                std::istringstream iss(trimmed.substr(3));
                iss >> eq_elem.epoch_mjd_tdb;
                found_mjd = true;
            }
            else if (trimmed.compare(0, 3, "MAG") == 0) {
                std::istringstream iss(trimmed.substr(3));
                iss >> eq_elem.magnitude >> eq_elem.mag_slope;
            }
            else if (trimmed.compare(0, 3, "COV") == 0) {
                std::istringstream iss(trimmed.substr(3));
                double val;
                while (iss >> val) {
                    cov_values.push_back(val);
                }
            }
            else if (trimmed.compare(0, 8, "! Object") == 0) {
                size_t obj_pos = trimmed.find(":");
                if (obj_pos != std::string::npos) {
                    size_t start = obj_pos + 1;
                    while (start < trimmed.length() && std::isspace(trimmed[start])) ++start;
                    size_t end = start;
                    while (end < trimmed.length() && !std::isspace(trimmed[end])) ++end;
                    eq_elem.object_name = trimmed.substr(start, end - start);
                }
            }
        }
        ...
        // Reconstruct covariance matrix if 21 values found (lower triangle)
        if (cov_values.size() == 21) {
            Eigen::Matrix<double, 6, 6> cov = Eigen::Matrix<double, 6, 6>::Zero();
            int idx = 0;
            for (int i = 0; i < 6; ++i) {
                for (int j = 0; j <= i; ++j) {
                    cov(i, j) = cov_values[idx++];
                    cov(j, i) = cov(i, j); // Symmetric
                }
            }
            eq_elem.covariance = cov;
        } else if (!cov_values.empty()) {
            std::cerr << "  ⚠ EQ1 Parser: found " << cov_values.size() << " covariance values, expected 21 (ignoring COV).\n";
        }
...
private:
    ...
    OrbitalElements equinoctial_to_keplerian(const EquinoctialElements& eq) {
        ...
        // Argument of perihelion: ω = atan2(h, k) - Ω
        ...
        // Mean anomaly: M = lambda - atan2(h, k)
