...
int main() {
    ...
    if (obs_res) {
        std::cout << "HZN-OBS: RA=" << obs_res->ra.value * constants::RAD_TO_DEG << " deg\n";
        std::cout << "HZN-OBS: Dec=" << obs_res->dec.value * constants::RAD_TO_DEG << " deg\n";
    }
    ...
    if (obs_res && vec_res) {
        auto earth_ssb = ephemeris::PlanetaryEphemeris::getPosition(ephemeris::CelestialBody::EARTH, t_ref);
        auto sun_ssb = ephemeris::PlanetaryEphemeris::getPosition(ephemeris::CelestialBody::SUN, t_ref);
        
        Eigen::Vector3d earth_helio = Eigen::Vector3d(earth_ssb.x, earth_ssb.y, earth_ssb.z) - 
                                      Eigen::Vector3d(sun_ssb.x, sun_ssb.y, sun_ssb.z);
        ...
        double dRA = (obs_res->ra.value - ra_calc) * constants::RAD_TO_DEG * 3600.0;
        double dDec = (obs_res->dec.value - dec_calc) * constants::RAD_TO_DEG * 3600.0;
