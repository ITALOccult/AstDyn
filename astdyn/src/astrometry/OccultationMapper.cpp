...
{
    // Distinct epoch types: TT drives precession-nutation, UT1 drives the Earth
    // rotation. Swapping them is now a compile error rather than a ~30 km one.
    const time::EpochTT  tt  = time::to_tt(t);
    const time::EpochUT1 ut1 = time::to_ut1(t);

    // The star direction is tagged GCRF, so shadow_point() can only compose it
    // with the full W*R*Q chain; ERA alone would not bind.
    const double a = ra.to_rad(), d = dec.to_rad();
    const auto star_dir = math::Direction<core::GCRF>::from_xyz(
        std::cos(d) * std::cos(a), std::cos(d) * std::sin(a), std::sin(d));

    const auto sp = coordinates::shadow_point(xi, eta, star_dir, tt, ut1);
    if (!sp) return std::nullopt;   // the axis misses the Earth

    // sp->lat is a GeodeticLatitude: a geocentric one could not have got here.
    return GeoPoint{sp->lat.angle(), sp->lon};
}
