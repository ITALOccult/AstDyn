/**
 * @file OrbFitAPI.cpp
 * @brief Implementation of OrbFitAPI
...
std::string OrbFitAPI::ltrim(const std::string& s) {
    size_t start = s.find_first_not_of(" \t");
    return (start == std::string::npos) ? "" : s.substr(start);
}

propagation::EquinoctialElements OrbFitAPI::parse_eq1(const std::string& filepath) {
    std::ifstream file(filepath);
    if (!file.is_open()) throw std::runtime_error("Cannot open file: " + filepath);

    propagation::EquinoctialElements equ; equ.epoch = time::EpochTDB::from_mjd(0.0);
    std::string line; bool found_equ = false, found_mjd = false;
    ...
    while (std::getline(file, line)) {
        ...
        if (trimmed.substr(0, 3) == "EQU") {
            ...
            equ.lambda *= constants::PI / 180.0; found_equ = true;
        } else if (trimmed.substr(0, 3) == "MJD") {
            std::istringstream iss(trimmed.substr(3)); double mjd_val;
            iss >> mjd_val; equ.epoch = time::EpochTDB::from_mjd(mjd_val); found_mjd = true;
        }
        if (found_equ && found_mjd) break;
    }
    if (!found_equ || !found_mjd) throw std::runtime_error("Could not parse equinoctial elements from file: " + filepath);
    equ.gravitational_parameter = constants::GMS; return equ;
}

types::OrbitalState<core::GCRF, types::KeplerianTag> OrbFitAPI::prepare_initial_state(const propagation::EquinoctialElements& mean_equ) {
    ...
    propagation::CartesianElements cart_eq; cart_eq.epoch = mean_equ.epoch;
    ...
    return types::OrbitalState<core::GCRF, types::KeplerianTag>(std::array<double, 6>{
        kep_eq.semi_major_axis, kep_eq.eccentricity, kep_eq.inclination,
        kep_eq.longitude_ascending_node, kep_eq.argument_perihelion, kep_eq.mean_anomaly
    });
}

OrbFitResult OrbFitAPI::run_fit(const std::string& eq1_file, const std::string& rwo_file, const std::string& config_file, bool verbose, const std::string& de441_path) {
    OrbFitResult result; result.success = false;
    try {
        if (verbose) std::cout << "[OrbFitAPI] Loading elements from " << eq1_file << "...\n";
        auto equ = parse_eq1(eq1_file); auto initial_state = OrbFitAPI::prepare_initial_state(equ);
        ...
        engine.set_initial_orbit(physics::KeplerianStateTyped<core::ECLIPJ2000>::from_traditional(
            equ.epoch, kep_ecl_old.semi_major_axis, kep_ecl_old.eccentricity,
        ...
        result.success = true; result.converged = engine_result.converged;
        result.fitted_state = types::OrbitalState<core::GCRF, types::KeplerianTag>(std::array<double, 6>{
            engine_result.orbit.a.to_au(), engine_result.orbit.e, engine_result.orbit.i.to_rad(),
            engine_result.orbit.node.to_rad(), engine_result.orbit.omega.to_rad(), engine_result.orbit.M.to_rad()
        });
        ...
        result.success = false; result.message = e.what();
    }
    return result;
}
