...
StateTransitionTensor::StateTransitionTensor(std::shared_ptr<const ForceField> force, double precision)
    : force_(std::move(force)), precision_(precision) {
...
double StateTransitionTensor::estimate_step(const Vector3d& q, const Vector3d& p,
    ...
    // Clamp di sicurezza sul passo: distanza dal corpo centrale (termine
    // dominante). I perturbatori entrano nella metrica via gradient_spectral_radius.
    double min_r = q.norm();
    if (!std::isfinite(min_r)) return 1e-8;

    const double sp = precision_ * 50.0;
    const double g_n = sp / std::sqrt(std::max(force_->gradient_spectral_radius(t, q), 1e-20));
    const Vector3d q_next = q + p * g_n;
    const double g_p = sp / std::sqrt(std::max(force_->gradient_spectral_radius(t, q_next), 1e-20));
    double g_avg = 2.0 * g_n * g_p / (g_n + g_p);

    const double T_min = astdyn::constants::TWO_PI
                       * std::sqrt(min_r * min_r * min_r / force_->settings().central_body_gm);
...
void StateTransitionTensor::kick(const Vector3d& q, Vector3d& p,
    ...
    // ForceField restituisce gia dA/dr = -U_ij e dA/dr2 = -U_ijk
    // (validato via differenze finite): nessuna negazione qui.
    const Matrix3d G = force_->acceleration_gradient(t, q, p);
    const Tensor3 T3 = force_->acceleration_second_gradient(t, q, p);
    p += h * force_->compute_acceleration(t, q, p);
...
void StateTransitionTensor::step(Vector3d& q, Vector3d& p,
    ...
    kick (q, p, phi, psi, d1_ * dt, t);
    drift(q, p, phi, psi, c2_ * dt);
    kick (q, p, phi, psi, d2_ * dt, t);
    drift(q, p, phi, psi, c2_ * dt);
    kick (q, p, phi, psi, d1_ * dt, t);
    drift(q, p, phi, psi, c1_ * dt);
}
...
StateTransitionTensor::propagate(const State& x0, time::EpochTDB t1) const {
    ...
    while (elapsed < total - 1e-12) {
        ...
        // Passo stimato con i perturbatori a inizio passo (via ForceField live).
        const double g = estimate_step(q, p, total - elapsed, t_now);

        // La forza valuta i perturbatori a META PASSO: errore del secondo
        // ordine invece del primo (Giove fa ~11 Mkm in 10 giorni). ForceField
        // interroga leffemeride live al tempo passato, quindi basta il tempo
        // di mezzo passo -- niente refresh esterno.
        const auto t_mid = t_now + time::TimeDuration::from_days(dir * 0.5 * g);
        step(q, p, phi, psi, dir * g, t_mid);
        elapsed += g;
        ++n_steps;
...
StateTransitionTensor::propagate_fixed(const State& x0, time::EpochTDB t1,
    ...
    const auto t0f = x0.epoch;
    for (int i = 0; i < n_steps; ++i) {
        const auto ti = t0f + time::TimeDuration::from_days(dt * i);
        step(q, p, phi, psi, dt, ti);
    }
    return Result{to_state(q, p, t1, x0.gm), phi, psi};
}
