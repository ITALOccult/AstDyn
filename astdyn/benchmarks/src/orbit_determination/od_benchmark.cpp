...
int main(int argc, char** argv) {
    ...
    for (const auto& tgt : targets) {
        ...
        if (tgt.warm_start) {
            ...
            if (!tgt.gooding) {
                iod_name = "OrbFitIOD";
                GaussIODSettings s; s.use_light_time = true; s.min_separation = time::TimeDuration::from_days(0.5);
                ...
                auto res = GoodingIOD(ephem, gs).compute(iod_obs[0], iod_obs[1], iod_obs[2], 
                    physics::Distance::from_au(tgt.r1), physics::Distance::from_au(tgt.r3));
