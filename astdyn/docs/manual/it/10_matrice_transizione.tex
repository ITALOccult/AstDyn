...
\begin{lstlisting}[language=C++,caption={Propagazione STM}]
...
prop.enable_stm(true);  // Abilita calcolo STM

// Propaga stato e STM
auto result = prop.propagate_with_stm(y0, t0, tf);

Vector6d yf = result.state;
Matrix6d Phi = result.stm;  // Matrice 6x6 di transizione stato

std::cout << "Determinante STM: " << Phi.determinant() << "\n";
std::cout << "Numero condizione STM: " 
          << Phi.norm() * Phi.inverse().norm() << "\n";
\end{lstlisting}
