...
To ensure stability even when starting from poor initial orbit guesses (e.g., RMS $> 1000$ arcsec), AstDyn implements specific safeguards:

\begin{description}
    \item[Trust Region (Step Limiting)] The correction vector $\Delta\mathbf{y}_0$ is clamped to a maximum magnitude (0.1 AU for position) per iteration. This prevents "overshooting" into physically invalid regions (e.g., hyperbolic escape) due to linearity breakdown.
    
    \item[Outlier Safety] Outlier rejection is disabled if the global RMS is very high ($> 100$ arcsec). This prevents the algorithm from discarding valid observations when the model is still far from convergence, allowing it to "walk" towards the solution first.
\end{description}
...
\STATE $k \leftarrow 0$
\REPEAT
    \STATE $\mathbf{N} \leftarrow \mathbf{0}, \mathbf{b} \leftarrow \mathbf{0}$
    \FOR{each observation $i$}
        \STATE Propagate to $t_i$: $[\mathbf{y}(t_i), \Phi(t_i, t_0)]$
        \STATE Compute prediction $\mathbf{c}_i = \mathbf{h}(\mathbf{y}(t_i))$ and residual $\boldsymbol{\xi}_i = \mathbf{o}_i - \mathbf{c}_i$
        \STATE Compute Partial $\mathbf{H}_i = \frac{\partial \mathbf{h}}{\partial \mathbf{y}(t_i)} \Phi(t_i, t_0)$
        \STATE Accumulate: $\mathbf{N} \leftarrow \mathbf{N} + \mathbf{H}_i^T \mathbf{W}_i \mathbf{H}_i$
        \STATE Accumulate: $\mathbf{b} \leftarrow \mathbf{b} + \mathbf{H}_i^T \mathbf{W}_i \boldsymbol{\xi}_i$
    \ENDFOR
    \STATE Solve $\mathbf{N} \Delta \mathbf{y}_0 = \mathbf{b}$
    \STATE Update $\mathbf{y}_0^{(k+1)} \leftarrow \mathbf{y}_0^{(k)} + \Delta \mathbf{y}_0$
    \STATE Compute new RMS $\sigma_{post}$
    \STATE $k \leftarrow k+1$
\UNTIL{Converged ($\|\Delta \mathbf{y}_0\| < \epsilon$) OR $k \ge k_{max}$}
\RETURN $\mathbf{y}_0^{(k)}$, Covariance $\mathbf{C} = \mathbf{N}^{-1}$
\end{algorithmic}
\end{algorithm}
...
struct DCResult {
    CartesianStateTyped<core::ECLIPJ2000> state;
    CovarianceMatrixAU<core::ECLIPJ2000> covariance;
    double rms;
    int iterations;
