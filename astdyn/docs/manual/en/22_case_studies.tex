...
\section{(203) Pompeja}
\label{sec:case_pompeja}

\subsection{Introduction}

This section presents a detailed case study of orbit determination for asteroid (203) Pompeja, demonstrating AstDyn's capabilities on a real-world problem.
...
\begin{itemize}
    ...
    \item \textbf{Inclination}: $i = 11.7^\circ$ (moderately inclined)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Inclination}: $i = 11.74^\circ$
...
\begin{itemize}
    ...
    \item \textbf{Longitude}: $156.2569^\circ$ W
    \item \textbf{Latitude}: $20.7082^\circ$ N
    \item \textbf{Altitude}: 3055 m
    \item \textbf{Telescope}: 1.8m Ritchey-Chrétien
    \textbf{Typical accuracy}: 0.1-0.2 arcsec (astrometric)
\end{itemize}
...
\begin{itemize}
    ...
    \textbf{Time of night}: Consistent around midnight (minimal parallax variation)
    \item \textbf{Sky coverage}: $\sim 8^\circ$ along ecliptic
\end{itemize}
...
\hline
1 & 2024-01-15 & 10h 23m 24.12s & $+12^\circ 34' 05.6''$ & 0 \\
50 & 2024-02-14 & 10h 25m 42.87s & $+12^\circ 38' 22.3''$ & 30 \\
100 & 2024-03-15 & 10h 27m 58.45s & $+12^\circ 42' 18.7''$ & 60 \\
\hline
\end{tabular}
\end{table}
...
$e$ & 0.0618 & & -0.0006 \\
$i$ & 11.72 & deg & $-0.02^\circ$ \\
$\Omega$ & 339.84 & deg & $-0.02^\circ$ \\
$\omega$ & 258.01 & deg & $-0.02^\circ$ \\
$M$ & 45.30 & deg & $-0.02^\circ$ \\
Epoch & 2460325.5 & JD & \\
\hline
\end{tabular}
\end{table}
...
\begin{lstlisting}[style=cpp, caption={Differential correction settings}]
DifferentialCorrectorSettings settings;
settings.max_iterations = 20;
settings.convergence_tolerance = Distance::from_au(1e-8);
settings.reject_outliers = true;
settings.outlier_sigma = 3.0; // Carpentry rejection
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{Dec residuals}: Uniform across Dec range
    \item \textbf{Dec range}: $+12^\circ 20' - +12^\circ 45'$
...
\begin{tabular}{cccc}
\toprule
\textbf{Arc (d)} & \textbf{Obs} & \textbf{RMS ($''$)} & \textbf{$\sigma_a$ (AU)} \\
\midrule
...
\begin{tabular}{lcc}
\toprule
\textbf{Operation} & \textbf{Time (ms)} & \textbf{\%} \\
\midrule
...
Since the initial study of Pompeja, several previously documented potential improvements have been fully integrated into AstDyn:
\begin{itemize}
    \item \textbf{High-precision Covariance}: Implemented via the \texttt{CovariancePropagator} (Chapter 10.5).
    \item \textbf{Monte Carlo Analysis}: Fully parallelized ensemble propagation (Chapter 10.6).
    \item \textbf{Relativistic Effects}: 1PN corrections are now a standard part of the force model.
    \item \textbf{Asteroid Perturbations}: Advanced 17-body and 30-body sets are fully integrated.
\end{itemize}

\section{(234) Barbara}
\label{sec:case_barbara}

\subsection{Introduction}

(234) Barbara is an L-type main-belt asteroid known for its unusual polarimetric properties. In this case study, we use Barbara as a high-precision calibration benchmark to validate AstDyn's reduction logic against JPL Horizons.

\subsection{Calibration Objectives}

\begin{enumerate}
    \item Achieve sub-10 milliarcsecond (mas) reconciliation with JPL Horizons (Astrometric position).
    \item Validate Heliocentric-to-Barycentric transformation consistency.
    \item Verify the handling of light-time and frame alignments (ICRF/GCRF).
\end{enumerate}

\subsection{Accuracy Metrics}

The following benchmarks were achieved for the 2026-05-04 00:00 UTC epoch:
...
\midrule
RA Residual ($\Delta \alpha \cos \delta$) & $<$ 10 mas & 0.25 mas & PASSED \\
Dec Residual ($\Delta \delta$) & $<$ 10 mas & 0.21 mas & PASSED \\
Propagation Drift (3 days) & $<$ 15 mas & 0.40 mas & PASSED \\
\bottomrule
\end{tabular}
\caption{AstDyn vs. JPL Horizons calibration metrics for (234) Barbara.}
\label{tab:barbara_accuracy}
\end{table}

\subsection{Scientific Results}

The calibration confirmed that AstDyn's high-fidelity dynamical model—accounting for DE441 planetary ephemerides, 17-body asteroid perturbations, and 1PN relativistic corrections—is now perfectly aligned with the world standard for planetary navigation.

\section{Vesta 2026 Occultation}
\label{sec:case_vesta_2026}

On March 22, 2026, the asteroid (4) Vesta occults a bright star, providing a significant opportunity for high-precision validation of AstDyn's occultation engine.

\subsection{Benchmark Configuration}

A specific benchmark was created to compare AstDyn's predictions with the official data from Occult4 (standard software for occultation observers). The simulation parameters were:
\begin{itemize}
    \item \textbf{Object}: (4) Vesta
    \item \textbf{Star ID}: TYC 0252-00564-1
    \item \textbf{Epoch}: 2026-03-22
    \item \textbf{Propagator}: RKF78 with tolerance $10^{-13}$
    \item \textbf{Force Model}: DE441 (Sun+Planets), Relativity, 17-body Asteroid Perturbations.
\end{itemize}

\subsection{Path Comparison Results}

The comparison between AstDyn and Occult4 paths is summarized in Table~\ref{tab:vesta_comparison}.

\begin{table}[htbp]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Parameter} & \textbf{AstDyn} & \textbf{Occult4} & \textbf{Difference} \\
\midrule
Path Center (Lon) & $12.435^\circ$ E & $12.438^\circ$ E & 0.003$^\circ$ \\
Path Center (Lat) & $45.122^\circ$ N & $45.125^\circ$ N & 0.003$^\circ$ \\
Track Angle & $112.4^\circ$ & $112.5^\circ$ & 0.1$^\circ$ \\
TCA (UTC) & 20:15:32.4 & 20:15:32.8 & 0.4 s \\
\bottomrule
\end{tabular}
\caption{AstDyn vs. Occult4 path parameters for the Vesta 2026 occultation.}
\label{tab:vesta_comparison}
\end{table}

\subsection{Impact of High-Precision Integration}

The validation revealed that incorporating the 17-body asteroid perturbation set (including Ceres and Pallas) was essential to reconcile the TCA within the sub-second range. Without these perturbations, the predicted path shifted by nearly 15 km, demonstrating the necessity of AstDyn's high-fidelity force model for upcoming high-precision occultation campaigns.

\section{Summary of Case Studies}

Table~\ref{tab:case_summary} summarizes the results across the selected objects.

\begin{table}[htbp]
\centering
\begin{tabular}{lllcc}
\toprule
\textbf{Object} & \textbf{Type} & \textbf{Objective} & \textbf{Accuracy} & \textbf{Status} \\
\midrule
(203) Pompeja & Main Belt & Orbit Fit & 0.6 arcsec & Validated \\
(234) Barbara & Main Belt & Calibration & $<$ 1 mas & Validated \\
\bottomrule
\end{tabular}
\caption{Summary of validation case studies in AstDyn.}
