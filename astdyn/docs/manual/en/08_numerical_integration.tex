...
\subsection{Phylogenetic Tree of Integrators}

The following tree (Figure~\ref{fig:integrator_tree}) illustrates the relationship between the various families of integrators implemented in AstDyn.
...
\begin{tikzpicture}[
  ...
  every node/.style={draw, rounded corners, fill=blue!10, font=\small, align=center}
]
\node {Numerical Integrators}
  child {node {One-Step (Iterative)}
    child {node {Explicit}
      child {node {RK4}}
      child {node {RKF78 / DOPRI54}}
    }
    child {node {Implicit}
      child {node {Radau IIA (Stiff)}}
      child {node {Gauss-Legendre\\(Symplectic)}}
    }
  }
  child {node {Specialized}
    child {node {Symplectic (Explicit)}
      child {node {SABA4}}
      child {node {AAS}}
    }
    child {node {Nyström (2nd Order)}
      child {node {GRKN}}
    }
  };
\end{tikzpicture}
\caption{Phylogenetic tree of numerical integrators in AstDyn.}
\label{fig:integrator_tree}
\end{figure}

\subsection{Integration Modes}

While numerical integrators provide the "engine" for propagation, the equations they solve depend on the chosen \textbf{Integration Mode}. AstDyn supports three primary modes:

\begin{enumerate}
    \item \textbf{Cowell's Method}: The most direct approach, where the full equations of motion $\ddot{\mathbf{r}} = \mathbf{a}_{\text{total}}$ are integrated in Cartesian coordinates. It is the default mode in AstDyn due to its flexibility and lack of singularity for any orbit.
    \item \textbf{Encke's Method}: Instead of the full position, only the deviation $\delta\mathbf{r}$ from a reference Keplerian conic is integrated. This is numerically superior for orbits that are nearly Keplerian (e.g., comets far from planets).
    \item \textbf{Multi-Body Integration}: For satellite systems (like Haumea's moons), AstDyn integrates the coupled equations of $N$ bodies simultaneously, accounting for mutual gravitational attraction and barycentric wobble.
\end{enumerate}
...
\subsection{Adaptive Adaptive Symplectic (AAS)}
\label{subsec:aas_integrator}

The AAS integrator is a high-performance symplectic method that employs a self-adaptive time-transformation (Sundman-style) to handle high-eccentricity orbits and close approaches without losing the symplectic property.

\subsubsection{Theoretical Basis}

Traditional adaptive step-size Runge-Kutta methods are not symplectic because the step-size change depends on the phase space coordinates in a non-canonical way. AAS solves this by introducing a fictitious time $s$ defined by:
\begin{equation}
    dt = g(\mathbf{q}) ds, \quad g(\mathbf{q}) = \frac{1}{\|\nabla U(\mathbf{q})\|}
\end{equation}
where $U(\mathbf{q})$ is the potential energy. This transformation scales the time steps such that they are smaller where the force gradient is higher.

\subsubsection{Algorithm}

AAS uses a 4th-order Wisdom-Holman splitting on the transformed Hamiltonian. It is particularly effective for systems where the primary body dominates the dynamics.

\subsubsection{Properties}

\begin{itemize}
    \item \textbf{Symplectic}: Yes (via time-transformed Hamiltonian)
    \item \textbf{Adaptivity}: Fully automatic based on force gradients
    \item \textbf{Efficiency}: Very high for planetary and satellite systems
    \item \textbf{Energy Drift}: Bounded (no secular growth)
\end{itemize}

\subsection{SABA4: Symplectic Order-4 Adaptive}
\label{subsec:saba4_integrator}

SABA4 is a 4th-order symplectic integrator from the SABA (Symplectic integrator for planetary systems) family, augmented with an adaptive step-size mechanism using a SABA2 embedding for error estimation.

\subsubsection{Algorithm}

The method uses the stages coefficients $c_i, d_i$ designed to minimize the leading order error term in the Hamiltonian splitting (Laskar \& Robutel, 2001).

\subsubsection{Properties}

\begin{itemize}
    \item \textbf{Order}: 4
    \item \textbf{Symplectic}: Yes (in fixed-step mode; approximately in adaptive mode)
    \item \textbf{Function evaluations}: 7 per step
    \item \textbf{Best usage}: Mid-term propagation of perturbed asteroid orbits
\end{itemize}
...
\section{Specialized Integrators}

AstDyn includes specialized integrators for specific mathematical structures, such as second-order systems and Variational Equations.

\subsection{Gauss-Runge-Kutta-Nystr\"om (GRKN)}
\label{subsec:grkn_integrator}

The GRKN method is a specialized Runge-Kutta-Nystr\"om integrator designed for second-order differential equations of the form $\ddot{\mathbf{r}} = \mathbf{f}(\mathbf{r}, t)$, where the acceleration does not explicitly depend on velocity (though it handles velocity-dependent perturbations via specialized stages).

\subsubsection{Key Features}

\begin{itemize}
    \item \textbf{Efficiency}: Reduces the number of function evaluations compared to reducing the system to first-order.
    \item \textbf{STM Propagation}: AstDyn uses GRKN to propagate the State Transition Matrix (STM) $\mathbf{\Phi}(t, t_0)$ simultaneously with the orbit.
    \item \textbf{Order}: 6th-order with 4th-order embedding for error control.
\end{itemize}

\subsubsection{STM Propagation in AstDyn}

When computing the Jacobian of the force field (Variational Equations), AstDyn employs the GRKN scheme to integrate:
\begin{equation}
    \dot{\mathbf{\Phi}}(t, t_0) = \mathbf{A}(t) \mathbf{\Phi}(t, t_0)
\end{equation}
This is critical for high-precision orbit determination and uncertainty mapping.

\subsection{Performance Comparison: RKF78 vs AAS}
\label{subsec:rkf78_vs_aas}

Recent benchmarks involving the propagation of TNOs and their satellite systems (e.g., Haumea) have highlighted the distinct advantages of the Adaptive Adaptive Symplectic (AAS) integrator compared to the general-purpose RKF78.

While RKF78 remains the robust default for arbitrary force models, AAS demonstrates superior accuracy in determining the \textbf{Time of Closest Approach (TCA)} during occultation events. When propagating Haumea's orbit using full numerical integration (bypassing native SPK ephemerides), AAS achieved a TCA reconciliation within \textbf{0.2 seconds} of the official SPK path, whereas RKF78 exhibited slightly larger secular drifts in the along-track component over multi-year arcs.

This performance gain is attributed to the symplectic nature of AAS combined with the Sundman-style time transformation, which naturally concentrates integration efforts near periapsis and close encounters without the overhead of the higher-order stage evaluations required by RKF78.
...
\midrule
RK4 & 4 & Explicit & 4 & \mystar\mystar\mystar\mystar\mystar & \mystar\mystar\mystar & Testing, debug \\
RKF78 & 7(8) & Explicit & 13 & \mystar\mystar\mystar\mystar & \mystar\mystar\mystar\mystar\mystar & General purpose \\
Radau15 & 15 & Implicit & $\sim$32 & \mystar\mystar\mystar & \mystar\mystar\mystar\mystar\mystar\mystar & Stiff, OD \\
Gauss & 8 & Symplectic & $\sim$16 & \mystar\mystar\mystar\mystar & \mystar\mystar\mystar\mystar & Long-term \\
AAS & 4(2) & Symplectic & 5 & \mystar\mystar\mystar\mystar\mystar & \mystar\mystar\mystar\mystar\mystar & Planets, moons \\
SABA4 & 4 & Symplectic & 7 & \mystar\mystar\mystar\mystar & \mystar\mystar\mystar\mystar & Asteroids \\
GRKN & 6(4) & Nystr\"om & 6 & \mystar\mystar\mystar\mystar\mystar & \mystar\mystar\mystar\mystar & STM, 2nd order \\
\bottomrule
\end{tabular}%
}
...
\begin{itemize}
    \item \textbf{General Occultation Prediction (< 30 days)}: RKF78
    \item \textbf{Planetary/Satellite Systems (High Precision)}: AAS
    \item \textbf{Orbit Determination (fitting observations)}: Radau15
    \item \textbf{State Transition Matrix (STM) Generation}: GRKN
    \item \textbf{Ephemeris Generation (1 year)}: RKF78 or AAS
    \item \textbf{Long-term Evolution (> 100 days)}: Gauss or SABA4
\end{itemize}
...
Table~\ref{tab:benchmark_results} shows performance for asteroid 17030 Sierks:
...
\subsubsection{AAS (Planetary/Satellite)}

\begin{lstlisting}[language=C++]
#include "astdyn/propagation/AASIntegrator.hpp"

// Precision 1e-12, include primary mu
AASIntegrator aas(1e-12, {mu_sun}); 

auto y_final = aas.integrate(derivative, y0, t0, tf);
\end{lstlisting}

\subsubsection{SABA4 (Symplectic Adaptive)}

\begin{lstlisting}[language=C++]
#include "astdyn/propagation/saba4_integrator.hpp"

SABA4Integrator saba(
    1.0,      // initial step
    1e-11,    // tolerance
    0.01,     // min step
    10.0      // max step
...
auto y_final = saba.integrate(derivative, y0, t0, tf);
\end{lstlisting}
...
\begin{enumerate}
    \item \textbf{Five core integrators} available: RKF78, Radau15, Gauss, AAS, SABA4, and GRKN
    \item \textbf{RKF78} is the default choice for general-purpose asteroid propagation
    \item \textbf{AAS} is the recommended choice for high-precision symplectic propagation of planetary/satellite systems
    \item \textbf{Radau15} provides highest accuracy for stiff problems and orbit determination
    \item \textbf{Gauss} is ideal for long-term, energy-conserving scientific simulations
    \item \textbf{GRKN} provides specialized performance for second-order systems and STM propagation
    \item \textbf{Adaptive step size} mechanisms in all modern AstDyn integrators balance accuracy and efficiency
\end{enumerate}
