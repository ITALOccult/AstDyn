...
\begin{equation}
g = 357.53^\circ + 0.9856003^\circ(JD - 2451545.0)
\end{equation}
...
The \texttt{astdyn::time} namespace provides strongly-typed epoch and conversion functions:
 
\begin{lstlisting}[style=cpp,caption={Time scale conversions}]
#include <astdyn/time/TimeScale.hpp>
using namespace astdyn::time;

// UTC to TDB conversion (Type-safe way)
EpochUTC t_utc = EpochUTC::from_mjd(61000.0);
EpochTDB t_tdb = to_tdb(t_utc);

// Access raw MJD value
double mjd_tdb = t_tdb.mjd();

// TT to TAI (raw double helpers also available)
double mjd_tt = 61000.0;
double mjd_tai = tt_to_tai(mjd_tt);

// UT1 to UTC (requires Delta_UT from IERS)
double delta_ut = 0.15;  // seconds
...
\subsection{\texorpdfstring{$\Delta T$}{Delta T} Approximations}
...
// Step 1: Calendar to MJD (UTC)
double mjd_utc_raw = calendar_to_mjd(year, month, day, hour/24.0);
EpochUTC t_utc = EpochUTC::from_mjd(mjd_utc_raw);

// Step 2: Convert to TDB (Strongly-typed conversion)
EpochTDB t_tdb = to_tdb(t_utc);
