...
\textbf{File sizes}:
\begin{itemize}
    \item DE440: 114 MB (standard planetary)
    \item DE441: 3.2 GB (long-term, planetary + 343 asteroids)
\end{itemize}

\subsection{Native SPK Provider}

AstDyn includes a native, dependency-free implementation for reading JPL DE440 and DE441 ephemerides directly from standard SPK binary kernels (.bsp files). This removes the need for the external CSPICE toolkit for high-precision planetary orbits and asteroid perturbations.

\textbf{Features}:
\begin{itemize}
    \item \textbf{Performance}: Up to 3x faster than traditional SPICE-based lookups due to direct memory mapping and optimized Chebyshev evaluators.
    \item \textbf{Type 2 \& 3 Support}: Full support for standard JPL planetary and asteroid SPK segments.
    \item \textbf{Zero-Dependency}: No shared libraries or external headers needed.
    \item \textbf{Thread-Safe}: Designed for parallel occultation searches.
\end{itemize}

\textbf{Configuration}:
\begin{lstlisting}[language=bash]
ephemeris.provider = NATIVE
ephemeris.file = /path/to/de441.bsp
\end{lstlisting}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Accuracy} & \textbf{Speed} & \textbf{File Size} & \textbf{Dependency} \\
\midrule
DE441 (Native) & 1--10 km & ★★★★★★ & 3.2 GB & None \\
DE440 (SPICE) & 1--10 km & ★★★★ & 114 MB & CSPICE \\
VSOP87 & 1--5 km & ★★★ & $\sim$1 MB & None \\
Keplerian & 100--1000 km & ★★★★★ & Negligible & None \\
\bottomrule
\end{tabular}
