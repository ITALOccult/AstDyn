...
\begin{itemize}
    \item \textbf{OrbFit .eq1}: Equinoctial elements from AstDyS/OrbFit
    \item \textbf{OrbFit .rwo}: Observation residuals and weights
    \item \textbf{MPC}: Astrometric observations in 80-column format
    \item \textbf{OEL}: High-precision orbital element exchange format (OEF2.0)
\end{itemize}
\section{Parser Interfaces}

AstDyn employs separate interfaces for orbital elements and astrometric observations to ensure type safety and clear separation of concerns.

\subsection{IOrbitParser}

Defined in \texttt{astdyn/io/IParser.hpp}, this interface handles the ingestion of initial states.

\begin{lstlisting}[language=C++,caption={IOrbitParser interface}]
...
public:
    ...
    // Returns strongly-typed Keplerian state
    virtual physics::KeplerianStateTyped<core::ECLIPJ2000> parse(
...
\subsection{IObservationParser}

This interface handles the ingestion of optical astrometry data.

\begin{lstlisting}[language=C++,caption={IObservationParser interface}]
...
public:
    ...
    virtual std::vector<OpticalObservation> parse(
        const std::string& filepath, size_t max_count = 0) = 0;
};
\end{lstlisting}
...
\begin{lstlisting}[language=C++,caption={ParserFactory usage}]
#include <astdyn/io/IParser.hpp>

// Create orbit parser (auto-detects .eq1)
auto orbit_parser = io::ParserFactory::create_orbit_parser("pompeja.eq1");
auto elements = orbit_parser->parse("pompeja.eq1");

// Create observation parser (auto-detects .rwo)
auto obs_parser = io::ParserFactory::create_observation_parser("data.rwo");
auto obs_list = obs_parser->parse("data.rwo");
\end{lstlisting}
