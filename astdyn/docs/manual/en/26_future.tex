...
\subsection{Yarkovsky and YORP Effects}

\textbf{Status}: Theoretical framework established; implementation in progress for v1.1.

\textbf{Planned}:
\begin{itemize}
    \item Diurnal and seasonal Yarkovsky models.
    \item Standard thermal model (STM) integration for diameter/albedo dependent forces.
    \item Parameter estimation (drift rate $da/dt$) in differential correction.
\end{itemize}

\textbf{Implementation sketch}:
\begin{lstlisting}[language=C++]
class YarkovskyForce : public ForceModel {
public:
    YarkovskyForce(double A2_scaled, double dAdt_estimated);
...
\textbf{Scientific impact}: Crucial for long-term impact risk assessment (e.g., Apophis, Bennu).

\subsection{Cometary Outgassing}
% ... (existing Marsden model content)

\section{Propagating Uncertainties}

\subsection{Current Implementation}

As of v1.0, AstDyn includes a complete suite of both linear and non-linear uncertainty propagation tools:
\begin{itemize}
    \item \textbf{CovariancePropagator}: Linear propagation via numeric or analytical STM, including process noise support.
    \item \textbf{MonteCarloUncertainty}: Non-linear ensemble propagation with OpenMP parallelization and full statistical reduction.
\end{itemize}

These features, previously planned as future enhancements, are now core components of the library and are detailed in Chapter~\ref{ch:state_transition}.

\subsection{Future Enhancements}

Planned extensions to the uncertainty engine include:
\begin{itemize}
    \item \textbf{Gaussian Mixture Models (GMM)}: For representing highly non-Gaussian distributions in non-linear regimes.
    \item \textbf{Covariance Refinement}: Automatic iterative refinement of the covariance matrix during orbit determination.
    \item \textbf{Process Noise Modeling}: Advanced models for integration noise and dynamical mismodeling.
\end{itemize}
...
\subsection{OpenMP and HPC}

\textbf{Status}: Implemented in v1.0 for batch trajectory exports and occultation searches.

\textbf{Planned}:
\begin{itemize}
    \item Fine-grained parallelization of the numerical integration steps (vectorization).
    \item Multi-threaded differential correction (parallel evaluation of the Jacobian).
    \item Native support for Apple Silicon Accelerate framework.
\end{itemize}
...
POST /api/v1/orbit_determination
  Body: { "observations": [...], "method": "diff_corr" }
  Returns: { "elements": {...}, "rms": 0.658 }

GET /api/v1/ephemeris?obj=pompeja&...
  Returns: [ { "epoch": 2460000.5, ... }, ... ]
\end{lstlisting}
...
Priority features:
\begin{itemize}
    \item Yarkovsky effect modeling and estimation.
    \item Python bindings (pybind11) complete release.
    \item Radar observations support and integration.
    \item Gaussian Mixture Models (GMM) for non-linear propagation.
\end{itemize}
...
\begin{itemize}
    \item Cometary outgassing model (Marsden).
    \item Symplectic integrators for long-term stability.
    \item IAU-CSN star catalog integration.
    \item Automated survey processing pipelines.
\end{itemize}
...
\begin{itemize}
    \item Kustaanheimo-Stiefel (KS) regularization.
    \item GPU-accelerated massive occultation searches.
    \item Cloud-native REST API deployment.
\end{itemize}
