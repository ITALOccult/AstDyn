...
// 2. Setup Jacobian Function (Analytical)
...
\begin{algorithmic}[1]
\REQUIRE Position $\mathbf{r}$
\ENSURE Jacobian $\mathbf{G} = \partial \mathbf{a} / \partial \mathbf{r}$
\STATE Initialize $\mathbf{G} = \mathbf{0}$
\STATE Add Keplerian term: $\mathbf{G} \leftarrow \mathbf{G} + \frac{\mu}{r^5}(3\mathbf{r}\mathbf{r}^T - r^2 \mathbf{I})$
\FOR{each planet $j$}
