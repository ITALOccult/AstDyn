...
The residuals (AstDyn - JPL) are shown in Figure~\ref{fig:residuals_val_precision}. The agreement is excellent: the maximum position error over the entire decade is below $2.5 \times 10^{-9}$ AU ($\approx 3.7$ meters).
...
\begin{figure}[htbp]
    ...
    \label{fig:residuals_val_precision}
\end{figure}

The statistical summary is provided in Table~\ref{tab:residuals_precision}.

\begin{table}[htbp]
\centering
\small
\caption{Residual statistics (AstDyn vs JPL DE441).}
\label{tab:residuals_precision}
\begin{tabular}{@{}lccc@{}}
\toprule
...
Figure~\ref{fig:benchmark_perf} benchmarks the performance of both methods.
