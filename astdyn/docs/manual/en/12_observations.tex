...
TimeDuration tau = TimeDuration::zero();  // Initial guess
for (int iter = 0; iter < 5; ++iter) {
    auto t_emit = t_obs - tau;
    auto state_emit = propagator.propagate(y0, t_emit);
    auto r_earth = ephemeris.get_position<core::GCRF>("Earth", t_obs);
    
    Distance rho = (state_emit.position - r_earth).norm();
    TimeDuration tau_new = TimeDuration::from_seconds(rho.to_m() / constants::SPEED_OF_LIGHT);
    
    if (std::abs((tau_new - tau).to_seconds()) < 1e-6) break;
    tau = tau_new;
}
\end{lstlisting}
