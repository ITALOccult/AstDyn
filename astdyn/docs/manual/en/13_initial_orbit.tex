...
CartesianStateTyped<Frame> gauss_iod(
    const std::array<Observation, 3>& obs,
    const EphemerisInterface& ephemeris)
{
    // Extract times (Strongly Typed)
    EpochTDB t1 = obs[0].epoch;
    EpochTDB t2 = obs[1].epoch;
    EpochTDB t3 = obs[2].epoch;
    
    auto rho_hat1 = obs[0].line_of_sight<Frame>();
    auto rho_hat2 = obs[1].line_of_sight<Frame>();
    auto rho_hat3 = obs[2].line_of_sight<Frame>();
    
    // Observatory positions [Distance]
    auto R1 = ephemeris.get_observer_position<Frame>(obs[0]);
    auto R2 = ephemeris.get_observer_position<Frame>(obs[1]);
    auto R3 = ephemeris.get_observer_position<Frame>(obs[2]);
    
    // Time intervals [TimeDuration]
    auto tau1 = t1 - t2;
    auto tau3 = t3 - t2;
    
    // Initial guess for middle range
    Distance rho2 = R2.norm();
    
    // Iterative refinement
    for (int iter = 0; iter < 20; ++iter) {
        auto r2_pos = R2 + (rho_hat2 * rho2.to_au()); 
        Distance r2_mag = r2_pos.norm();
        
        // ... (Compute f,g coefficients and solve for v2)
    }
