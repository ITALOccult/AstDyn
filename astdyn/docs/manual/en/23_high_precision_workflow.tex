...
\section{Simplified Integration: ITALOccultLibrary}
\label{sec:italoccultlib_integration}

To facilitate the use of these features in larger projects like \texttt{IOccultCalc}, the \texttt{ITALOccultLibrary} was developed as a simplified wrapper.

\subsection{Automatic Mean-to-Osculating Conversion}
A common pitfall is using mean orbital elements from AstDyS (\texttt{.eq1} files) directly in an N-Body numerical propagator. This typically results in a systematic error of $\sim 360$ arcseconds. The \texttt{AstDynWrapper} automatically handles this by applying the rigorous conversion at load-time:

\begin{lstlisting}[style=cpp, caption={AstDynWrapper for High-Precision}]
#include <italoccultlib/astdyn_wrapper.h>

ioccultcalc::AstDynWrapper wrapper;
// Automatically parses and converts Mean (Ecliptic) -> Osculating (Equatorial)
wrapper.loadFromEQ1File("34713.eq1");

// Compute geocentric observation with full DE441 part-2 perturbations
