...
\begin{tikzpicture}[scale=1.0]
    ...
    \shade[ball color=blue!5!white, opacity=0.3] (0,0) circle (2.5cm);
    
    % Equator (Blue)
    \draw[blue, thick] (-2.5,0) arc (180:360:2.5 and 0.8);
    \draw[blue, dashed] (2.5,0) arc (0:180:2.5 and 0.8);
    \node[blue, right] at (2.5,0) {Equator};
    
    % Ecliptic (Red, tilted 23.5 degrees)
    \begin{scope}[rotate=23.5]
        \draw[red, thick] (-2.5,0) arc (180:360:2.5 and 0.8);
        \draw[red, dashed] (2.5,0) arc (0:180:2.5 and 0.8);
        \node[red, above right] at (2.5,0) {Ecliptic};
    \end{scope}

    % North Pole
    \draw[->, thick] (0,0) -- (0,3.0) node[above] {NCP};
    \draw[dashed] (0,0) -- (0,-2.5);
    
    % Vernal Equinox (Gamma)
    % See Figure~\ref{fig:precession_coords} for a visual representation.
    \filldraw (0, -0.8) circle (1.5pt) node[below left] {$\gamma$};
    
    % Angle epsilon
    \draw[<->] (2.8, 0) arc (0:23.5:2.8);
    \node at (3.0, 0.4) {$\varepsilon$};

    % Star vector
    \draw[->, thick, black!70] (0,0) -- (1.5, 1.8) node[midway, fill=white, inner sep=1pt] {$r$} node[above right] {Star};
    \fill (1.5, 1.8) circle (1.5pt);
    
\end{tikzpicture}
\caption{The Celestial Sphere showing the Equatorial (blue) and Ecliptic (red) planes. The angle between them is the obliquity of the ecliptic $\varepsilon \approx 23.4^\circ$.}
...
// Ecliptic to J2000 equatorial (Strongly Typed)
auto pos_ecl = math::Vector3<core::ECLIPJ2000, Distance>::from_au(1.0, 0.5, 0.1);

// Type-safe transformation (Compile-time frame check)
auto pos_eq = ReferenceFrame::transform_pos<core::ECLIPJ2000, core::GCRF>(pos_ecl);

// Velocity transformation
auto vel_eq = math::Vector3<core::GCRF, Velocity>::from_au_d(0.01, 0.02, 0.005);
auto vel_ecl = ReferenceFrame::transform_vel<core::GCRF, core::ECLIPJ2000>(
    pos_eq, vel_eq
);
\end{lstlisting}
...
\centering
\begin{tikzpicture}[scale=1.0]
    % Same base size as the sphere (radius 2.5) for consistency
    % Ecliptic Pole
    \draw[->, thick] (0,0) -- (0,3.0) node[above] {NEP};
    
    % Precession Cone
    \draw[dashed] (0,2.2) ellipse (0.9 and 0.25); % Cone top
    \draw[dashed] (0,0) -- (-0.9, 2.2);
    \draw[dashed] (0,0) -- (0.9, 2.2);
    
    % Earth Axis (NCP)
    \draw[->, thick, blue] (0,0) -- (0.9, 3.0) node[right] {NCP (J2000)};
    \draw[->, thick, red, dashed] (0,0) -- (-0.9, 3.0) node[left] {NCP (Future)};
    
    % Direction
    \draw[->] (0.5, 2.5) arc (15:165:0.5 and 0.15);
    \node at (0, 2.8) {\scriptsize 26,000 yr};

    % Base Ecliptic Plane (Visualize it same size as Fig 1)
    \draw (0,0) ellipse (2.5 and 0.6);
    \node at (2.5,0.5) {Ecliptic Plane};
    
    % Earth at center
    \shade[ball color=blue!50] (0,0) circle (0.3);
\end{tikzpicture}
\caption{Precession of the Earth's rotation axis (NCP) around the North Ecliptic Pole (NEP).}
\label{fig:precession_coords}
\end{figure}
