...
\begin{tikzpicture}[scale=1.2, >=stealth]
    % COORDINATE PUNTI CHIAVE (Key Coordinates)
    \coordinate (Sun) at (0,0);
    \coordinate (AscNode) at (3,0);
    \coordinate (DescNode) at (-3,0);

    % 1. PIANO DI RIFERIMENTO (Reference Plane)
    % Ellisse grigia centrata nel Sole passante per i nodi
    \draw[thick, gray!50] (0,0) ellipse (3.5cm and 1.0cm);
    \node[gray!80!black, anchor=north east] at (3.5,-0.5) {Reference Plane};

    % 2. SOLE (Sun)
    \filldraw[orange] (Sun) circle (4pt);
    \node[below=8pt] at (Sun) {Sun (Focus)};

    % 3. LINEA DEI NODI (Line of Nodes)
    % Congiunge Nodo Discendente e Ascendente passando per il Sole
    \draw[dashed, black!60] (DescNode) -- (AscNode);
    \filldraw[black] (AscNode) circle (2pt);
    \node[right=3pt] at (AscNode) {Ascending Node};
    
    % 4. DIREZIONE GAMMA (Reference Direction)
    % Disegnata sul piano di rif. a -20 gradi
    \draw[->, blue, thick] (Sun) -- (2.8, -0.95) node[right] {$\gamma$};
    
    % ANGOLO OMEGA (Longitude of Asc Node)
    % Da Gamma a Nodo Ascendente (asse X)
    \draw[->, purple, thick] (1.2, -0.4) arc (-18:0:1.25);
    \node[purple, right] at (1.3, -0.2) {$\Omega$};

    % 5. ORBITA (Orbit)
    % Disegnata "al di sopra" e "al di sotto" della linea dei nodi per simulare inclinazione
    % Parte superiore (Orbitale) passante per i nodi
    \draw[thick, red] (AscNode) .. controls (3.0, 2.5) and (-2.0, 3.0) .. (DescNode);
    
    % Parte inferiore (Orbitale - tratteggiata per profondità)
    \draw[dashed, red] (DescNode) .. controls (-3.5, -1.5) and (3.0, -1.5) .. (AscNode);

    % 6. PERIELIO (Perihelion)
    % Punto più vicino al Sole sulla curva rossa
    \coordinate (Perihelion) at (1.5, 1.8); 
    \filldraw[red] (Perihelion) circle (2pt);
    \node[above, red] at (Perihelion) {Perihelion ($\omega$)};
    \draw[dashed, red] (Sun) -- (Perihelion);

    % ANGOLO omega (Argomento Perielio)
    % Dal Nodo al Perielio
    \draw[->, orange] (0.8, 0) arc (0:50:0.8);
    \node[orange, above right] at (0.8, 0.4) {$\omega$};

    % 7. CORPO CELESTE (Body)
    \coordinate (Body) at (-0.5, 2.2);
    \filldraw[blue] (Body) circle (3pt) node[above] {Body};
    \draw[dotted, blue] (Sun) -- (Body) node[midway, fill=white, inner sep=1pt] {$r$};

    % True Anomaly (nu)
    \draw[->, blue] (0.6, 0.7) arc (50:100:0.6);
    \node[blue, above] at (0.2, 0.8) {$\nu$};

    % 8. INCLINAZIONE (i)
    % Indicatore visivo del tilt al nodo
    \draw[green!40!black, thick] (2.4, 0) arc (180:140:0.8);
    \node[green!40!black, left] at (2.4, 0.3) {$i$};
...
// 1. Create strongly-typed Keplerian state (Ecliptic J2000)
auto epoch = EpochTDB::from_jd(2460000.0);
auto kep = KeplerianStateTyped<core::ECLIPJ2000>::from_traditional(
    epoch, 
    2.77,   // a [AU]
    0.078,  // e
    10.6,   // i [deg]
...
// 2. Convert to Cartesian State (Strongly Typed)
CartesianStateTyped<core::ECLIPJ2000> sv = kep.to_cartesian();

std::cout << "Position: " << sv.position.to_eigen_si().transpose() << " m\n";
std::cout << "Velocity: " << sv.velocity.to_eigen_si().transpose() << " m/s\n";

// 3. Convert back to Keplerian (Inverse transform)
auto kep2 = KeplerianStateTyped<core::ECLIPJ2000>::from_cartesian(sv);

// Verify round-trip (AU conversion)
std::cout << "Delta a: " << (kep2.a - kep.a).to_au() << " AU\n";
// Output: Delta a: 0.0 AU (exact)

// 4. Access helper methods
std::cout << "Period: " << kep.period_days() << " days\n";
\end{lstlisting}
