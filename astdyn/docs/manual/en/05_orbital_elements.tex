...
\begin{tikzpicture}[scale=1.0]
    % Reference plane (ecliptic or equator) - Standardized size (2.5-3.0 scale)
    \draw[thick,blue!20,fill=blue!5] (-3.5,-1.5) -- (3.5,-1.5) -- (3.5,1.5) -- (-3.5,1.5) -- cycle;
    \node[blue] at (2.8,-1.2) {Reference plane};
    ...
    \begin{scope}[rotate around={15:(0,0)}]
        % Ellipse - Standardized to ~2.5cm major radius
        \draw[thick,red,rotate=20] (0,0) ellipse (2.5cm and 1.3cm);
        
        % Perihelion
        \filldraw[red] (-2.0,0.7) circle (2pt);
        \node[red,above] at (-2.0,1.0) {Perihelion};
        
        % Ascending node (Intersection approx)
        \filldraw[green!60!black] (1.3,-0.6) circle (2pt);
        \node[green!60!black,below] at (1.3,-0.9) {Ascending node};
        
        % Longitude of ascending node (Ω) - Arc on reference plane actually
        % But here we draw relative angle components
        
        % Argument of perihelion (ω)
        \draw[->,orange,thick] (1.3,-0.6) arc (-25:40:1.0);
        \node[orange] at (0.4,0.3) {$\omega$};
        
        % Inclination line visual
        \draw[dashed,green!60!black] (1.3,-1.5) -- (1.3,1.5);
    \end{scope}
    
    % Reference direction (vernal equinox) - Length 3.5 to match plane
    \draw[->,thick,blue] (0,0) -- (3.5,0);
    \node[blue,right] at (3.5,0) {$\gamma$};
    
    % Longitude of Ascending Node (Omega) on Ref Plane
    \draw[->,purple,thick] (1.5,0) arc (0:-20:1.5);
    \node[purple] at (1.6,-0.3) {$\Omega$};
